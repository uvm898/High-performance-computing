...
__global__ void feynman(const int N, float *params, float *results, const int a, const int b, const int c, const float h, const float stepsz)
{
  curandState rand;
...
float par_exe(const int N, float *elapsed_time)
{
  ...
  // CALL KERNEL HERE
  feynman<<<n_inside, NUM_OF_GPU_THREADS>>>(N, gpu_coordinates_arr, gpu_results_arr, a, b, c, h, stepsz);
